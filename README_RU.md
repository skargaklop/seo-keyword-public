...
CHANGE_SUMMARY: Added GRACE documentation module contract linking README to MOD-DOC. Refreshed browser-scraping/Trends-provider framing — browser scraping is the default provider for Google Trends and SERP, enabled by default.
-->
...
- **Google Trends** — анализ Google Trends с interest over time, related queries и региональными данными (стandalone или как optional stage; related topics только через провайдеры SerpApi/DataForSEO)
...
- **SERP Analysis**: многопровайдерный клиент с адаптерами для Serper, SerpApi, Brave, Zenserp, SearchApi.io, ScraperAPI, DataForSEO, Serpstat, Serpstack, ScaleSERP и ValueSERP, плюс браузерный адаптер (`browser_cloakbrowser`) без необходимости в API-ключе
...
- **Google Trends integration**:
  ...
  - Interest over time, related queries, region data (related topics только через провайдеры SerpApi/DataForSEO; браузерный провайдер по умолчанию их не поддерживает)
  - Относительные значения 0-100 с опциональным anchor rescaling
  - Rate limiting и кэширование для избежания 429 errors
- **OPT-IN browser scraping**: провайдер по умолчанию для Google Trends и SERP — browser-based scraping через Cloakbrowser (включён по умолчанию); API-провайдеры — опциональная альтернатива при наличии API-ключа
...
- Установите `scraper.browser_enabled: true` в `config/settings.yaml` или через sidebar (это значение по умолчанию)
- Browser scraping — провайдер по умолчанию для SERP и Trends; API-ключ не требуется для базовой функциональности
...
| `SERPAPI_KEY` | SerpApi — коммерческий Trends-провайдер `serpapi_trends` (также указан в SERP; поддерживает related topics) |
| `DATAFORSEO_LOGIN` + `DATAFORSEO_PASSWORD` | DataForSEO — коммерческий Trends-провайдер `dataforseo_trends` (также указан в SERP; поддерживает related topics) |
...
4. Запустите анализ для получения interest over time, related queries и regional данных (related topics только через провайдеры SerpApi/DataForSEO)
...
- **Механика провайдера по умолчанию**: провайдер по умолчанию `browser_scraper_trends` скачивает CSV-экспорт Google Trends через browser automation (Cloakbrowser) для interest-over-time timeline, а related queries и региональные данные восстанавливает парсингом отрендеренного HTML Trends, когда они доступны. Related topics этим провайдером не поддерживаются — используйте SerpApi или DataForSEO для тем.
- **Уровни уверенности (confidence)**: браузерный провайдер по умолчанию сообщает `MEDIUM` при полном успехе, `LOW` при пустом CSV timeline, `BLOCKED` при HTTP 429/captcha и `DEGRADED` при частичных неудачах multi-keyword. Включите `google_trends.show_confidence_metadata`, чтобы их отображать.
...
Browser-based scraping — провайдер по умолчанию для Google Trends (`browser_scraper_trends`) и SERP (`browser_cloakbrowser`); API-key провайдер — опциональная альтернатива при наличии API:
...
### Anti-Bot Handling
- Browser engine (Cloakbrowser) обрабатывает CAPTCHAs и basic anti-bot
